 ...
 */
public class SlideGlowPadView extends View {
...
//	private PointCloud mPointCloud;
	...
	private float mDistSlide;
	private float mHeightSlide;
	private int mPointerId;
	private boolean mShowTargetsOnIdle;

	public SlideGlowPadView(Context context) {
		this(context, null);
	}

	public SlideGlowPadView(Context context, AttributeSet attrs) {
		super(context, attrs);
		Resources res = context.getResources();

		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.SlideGlowPadView);
		mInnerRadius = a.getDimension(R.styleable.SlideGlowPadView_innerRadiusSlide, mInnerRadius);
//		mOuterRadius = a.getDimension(R.styleable.GlowPadView_outerRadius, mOuterRadius);
		mDistSlide = a.getDimension(R.styleable.SlideGlowPadView_distenceSlide, mDistSlide);
		mHeightSlide = a.getDimension(R.styleable.SlideGlowPadView_heightSlide, mHeightSlide);
		mSnapMargin = a.getDimension(R.styleable.SlideGlowPadView_snapMarginSlide, mSnapMargin);
		mVibrationDuration = a.getInt(R.styleable.SlideGlowPadView_vibrationDurationSlide, mVibrationDuration);
		mFeedbackCount = a.getInt(R.styleable.SlideGlowPadView_feedbackCountSlide, mFeedbackCount);
		TypedValue handle = a.peekValue(R.styleable.SlideGlowPadView_handleDrawableSlide);
...
//		mOuterRing = new TargetDrawable(res, getResourceId(a, R.styleable.GlowPadView_outerRingDrawable), 1);
		...
		mAlwaysTrackFinger = a.getBoolean(R.styleable.SlideGlowPadView_alwaysTrackFingerSlide, false);
...
//		mGlowRadius = a.getDimension(R.styleable.GlowPadView_glowRadius, 0.0f);
		...
		if (a.getValue(R.styleable.SlideGlowPadView_targetDrawablesSlide, outValue)) {
		...
		if (a.getValue(R.styleable.SlideGlowPadView_targetDescriptionsSlide, outValue)) {
		...
		if (a.getValue(R.styleable.SlideGlowPadView_directionDescriptionsSlide, outValue)) {
			final int resourceId = outValue.resourceId;
			if (resourceId == 0) {
...
//		a.recycle();
		...
		mGravity = a.getInt(R.styleable.SlideGlowPadView_android_gravity, Gravity.TOP);
		a.recycle();
...
//		mGlowAnimations.start();
	...
	protected int getSuggestedMinimumWidth() {
		...
		return (int) (Math.max(getWidth(), 2 * mDistSlide) + mMaxTargetWidth);
//		return (int) (2 * mDistSlide + mMaxTargetWidth);
	}
	...
	protected int getSuggestedMinimumHeight() {
		...
		// target drawable on either edge
		return (int) (Math.max(getHeight(), mHeightSlide));
//		return (int) mHeightSlide;
	}
...
//			hideGlow(0, 0, 0.0f, null);
			...
			updateHandleState(TargetDrawable.STATE_INACTIVE);
			updateHandlePosition(mWaveCenterX, mWaveCenterY);
		...
		case STATE_FIRST_TOUCH:
			updateHandleState(TargetDrawable.STATE_ACTIVE);
			updateHandlePosition(x, mWaveCenterY);
			deactivateTargets();
			if (mShowTargetsOnIdle) {
				showTargets(false);
			} else {
...
//			showGlow(REVEAL_GLOW_DURATION, REVEAL_GLOW_DELAY, 1.0f, null);
		...
		case STATE_SNAP:
			...
			updateHandleState(TargetDrawable.STATE_FOCUSED);
			updateHandlePosition(x, mWaveCenterY);
...
//		Log.d(TAG, "----X position---->" + event.getX(actionIndex) + "----Y position---->" + event.getY(actionIndex));
		switch (action) {
...
//	}
	...
	private void updateHandlePosition(float x, float y) {
		mHandleDrawable.setPositionX(x);
		mHandleDrawable.setPositionY(y);
		Log.d(TAG, "----X position---->" + x + "----Y position---->" + y);
	}
	
	private void updateHandleState(int[] state) {
		if (mHandleDrawable != null) {
			if (state == TargetDrawable.STATE_ACTIVE) {
				mHandleDrawable.setState(state);
				mHandleDrawable.resizePressedHandle(true);
				mHandleDrawable.setAlpha(1.0f);
			} else if (state == TargetDrawable.STATE_INACTIVE) {
				mHandleDrawable.setState(state);
				mHandleDrawable.resizePressedHandle(false);
				mHandleDrawable.setAlpha(1.0f);
			} else {
	...
	private void handleDown(MotionEvent event) {
		...
		if (!trySwitchToFirstTouchState(eventX, eventY)) {
			...
			updateHandleState(TargetDrawable.STATE_ACTIVE);
			updateHandlePosition(eventX, eventY);
			Log.d(TAG, "----X position down---->" + eventX + "----Y position down---->" + eventY);
...
//			final float scale = tx > mDistSlide ? mDistSlide / tx : 1.0f;
//			float limitX = tx * scale;
//			float limitY = ty * scale;
//			double angleRad = Math.atan2(-ty, tx);

			if (!mDragging) {
				trySwitchToFirstTouchState(eventX, mWaveCenterY);
			}

			if (mDragging) {
				// For multiple targets, snap to the one that matches
				final float snapDist = mDistSlide - mSnapMargin;
//				final float snapDistance2 = snapRadius * snapRadius;
				// Find first target in range
				for (int i = 0; i < ntargets; i++) {
					TargetDrawable target = targets.get(i);

					double targetRad = (i - 0.5) * 2;
					if (target.isEnabled()) {
						boolean distMatches = Math.abs(tx) >= snapDist;
						boolean angleMatches = (tx<=0 && targetRad<=0) || (tx>=0 && targetRad>=0);
						if ((distMatches && angleMatches)) {
							activeTarget = i;
						}
					}
				}
...
//			y = limitY;
		...
		if (activeTarget != -1) {
			switchToState(STATE_SNAP, event.getX(actionIndex), event.getY(actionIndex));
//			updateGlowPosition(x, y);
		} else {
			switchToState(STATE_TRACKING, event.getX(actionIndex), event.getY(actionIndex));
//			updateGlowPosition(x, y);
		}
		...
		if (mActiveTarget != activeTarget) {
			...
			if (activeTarget != -1) {
//				mHandleDrawable.setAlpha(0.0f);
//				mHandleDrawable.setEnabled(false);
//				switchToState(STATE_IDLE, event.getX(actionIndex), event.getY(actionIndex));
...
//				final AccessibilityManager accessibilityManager = (AccessibilityManager) getContext().getSystemService(Context.ACCESSIBILITY_SERVICE);
	...
	private boolean trySwitchToFirstTouchState(float x, float y) {
		...
		if (mAlwaysTrackFinger || dist2(tx, ty) <= square(mInnerRadius)) {
			if (DEBUG)
...
//			updateGlowPosition(tx, ty);
	...
	private void assignDefaultsIfNeeded() {
		if (mDistSlide == 0.0f) {
			mDistSlide = getWidth() / 3.0f;
		}
		if (mHeightSlide == 0.0f) {
			mHeightSlide = mDistSlide;
		}
		if (mSnapMargin == 0.0f) {
...
//		final float placementHeight = Math.max(mOuterRing.getHeight(), 2 * mOuterRadius);
		float newWaveCenterX = mHorizontalInset + Math.max(width, mMaxTargetWidth + 2 * mDistSlide) / 2;
		float newWaveCenterY = mVerticalInset + Math.max(height, mHeightSlide) / 2;
...
//			final float angle = alpha * i;
			...
			targetIcon.setX(mDistSlide * (float) (i - 0.5) * 2);
			targetIcon.setY(0);
		}
	}
